...
extract_scores_UNIVFPCA <- function(mZ1, mZ2, tt, PVE = 0.95) {
  ...
  save(mZ1, mZ2, tt, PVE, out1, out2, file = "fpca_1_2.RData")
...
cfda_score_function <- function(cfda_data, nCores, timestamps01, basis_num) {
  # 1. Data preparation
  cfda_data <- t(cfda_data) # Transpose to individuals × time
  ...
  # 2. Reshape to long format for CFDA
  cfda_new <- data.frame(cbind(cfda_data, subjects))
  data_long <- gather(cfda_new, time_t, state,
    paste0("time_", times[1]):paste0("time_", times[timeseries_length]),
    factor_key = TRUE
  )
  cfda_d <- data_long %>%
    arrange(subjects) %>%
  ...
  # 4. Component selection based on PVE
  delta <- 0.01
  pve <- 0.95
